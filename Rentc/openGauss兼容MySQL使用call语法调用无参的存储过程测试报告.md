...
| 版本名称 | 测试起始时间 | 测试结束时间 |
| --- | --- | --- |
| mysql 5.7 | 2022-09-20 | 2022-09-24 |
| openGauss 3.0.0 build b410c303 | 2022-09-20 | 2022-09-24 |
| dolphin 1.0 | 2022-09-20 | 2022-09-24 |
...
openGauss兼容MySqlcall在调用无参的存储过程共计执行用例15条，主要覆盖了功能测试。功能测试覆盖在创建自定义的无参函数和存储过程，在其中有嵌套各种自定义函数、数据库已有函数和存储过程。函数、存储过程的实现内容包括if loop while loop等条件判断、事务、ddl、dql、dml、exception等。同时包含数据库导入导出和创建新用户调用等场景。目前并未发现问题单，特性的质量良好。
...
| 测试步骤 | 测试结果 |
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2.在存储过程中添加事务、条件判断、exception等 | 执行5条用例，执行结果符合预期，未发现bug |
...
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2.存储过程在函数、存储过程中嵌套调用 | 执行3条用例，执行结果符合 |
...
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2.在存储过程中添加事务、条件判断、exception等 | 执行5条用例，执行结果符合预期 |
...
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2. 在存储过程，函数中执行ddl、dml操作 | 执行3条用例，执行结果符合预期 |
...
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2. 异常（省略括号）调用有参的存储过程 | 执行1条用例，执行结果符合预期 |
...
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2.异常在不同兼容模式下（TD、PG、Oracle)使用call省略括号调用无参的存储过程 | 执行3条用例，执行结果符合预期 |
...
| --- | --- |
| 1. 在兼容B库并安装dolphin插件的环境 
2.  测试函数在文档中的说明是否准确，示例的执行结果是否正确 | 符合预期 |
...
| --- | --- | --- | --- |
| openGauss 3.0.0 build b410c303 | 15 | Passed：15
Failed：0 | 0 |

...
（1）除了随机单表外，其表类型对应视图创建，需要配置视图分片规则（同表的分片规则一致），同时需要配置视图与表的绑定关系（视图与表的分片规则应当一致）；

（2）广播表创建的视图，需要将对应视图配置为广播表规则；

（3）多表关联、分片表group by ..having复杂查询等跨库场景，需要用户自行保证跨库的正确性；

（4）创建视图中的query语句必须简单，不支持聚合、子查询、order by等复杂语句；

（5）ss-proxy5.1.3 snapshot 及以上版本具备 view 功能。
...
- 最终测试用例执行通过25个，不通过0个；
- 缺陷密度为10(缺陷个数)/1.0k(代码行数)=10(个/kloc)。

...
| 软件名称     |                                              |
| ------------ | -------------------------------------------- |
| tidRangescan | 获取地址：https://gitee.com/opengauss/Plugin |
...
| 性能测试   | （1）验证100G数据量下，对比直连openGauss、OLK直连openGauss执行tpch查询sql，OLK配合使用TID插件，并发执行tpch查询sql的性能提升；<br />（2）验证100G数据量下，OLK配合使用TID插件，openGauss部署单机、1主1备和1主2备，并发执行tpch查询sql，随着备机增加性能提升；<br/>（3）验证增加分片数时，OLK配合使用TID插件，并发执行tpch查询sql，随着分片数的增加性能提升；<br/>（4）验证增加并发度，OLK配合使用TID插件，并发执行tpch查询sql，随着并发度的增加性能提升；<br/>**整体结论：对比直连openGauss、OLK直连openGauss，OLK配合TID插件性能有很大提升；但随着备机的增加，性能没有达到预期的提升，遗留处理，后续优化解决。** |

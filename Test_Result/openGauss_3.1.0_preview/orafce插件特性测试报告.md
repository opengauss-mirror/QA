...
orafce for openGauss 共计执行150条用例，主要覆盖功能测试。功能包含兼容O*的包、函数、系统视图等特性。资料测试覆盖校验资料的描述及示例的执行结果。发现问题均已解决，回归通过，无遗留风险，整体质量良好。
...
说明：发现9个问题单，7条已修改并验证通过，1条非问题已取消，1条遗留问题状态为挂起。

...
openGauss兼容mysql IS_IPV4/IS_IPV6/SET PASSWORD/KILL语法共计执行用例40条，主要覆盖了功能测试和资料测试。功能测试主要覆盖语法中使用不同数量的参数以及参数的有效和无效取值，基于不同权限的用户执行SET PASSWORD语句，基于不同权限的用户执行KILL语句，基于线程池模式和非线程池模式执行KILL语句，资料测试覆盖资料描述是否准确、约束覆盖是否全面以及示例是否正确。累计发现缺陷单3个，1个确认非问题取消，2个待办的，其中一个由于本地未复现，由CCB裁决处理。
...
| 2    | [I5N7Y5](https://gitee.com/opengauss/Plugin/issues/I5N7Y5?from=project-issue) | 次要     | 执行kill语句后返回结果与预期结果不符                         | 待办的   |
| 3    | [I5S8T0](https://gitee.com/opengauss/Plugin/issues/I5S8T0?from=project-issue) | 次要     | dolphin插件doc下《SET-PASSWORD.md》中注意事项表述不准确      | 待办的   |
...
| 测试步骤：                       | 测试结果                                                     |
| -------------------------------- | ------------------------------------------------------------ |
| 1.基于不同权限的用户执行KILL语句 | 执行2条用例，发现1个bug，目前状态待办的，由于本地未复现，由CCB裁决处理 |
...
| 测试步骤：                                           | 测试结果                     |
| ---------------------------------------------------- | ---------------------------- |
| 1.资料描述是否准确、约束覆盖是否全面以及示例是否正确 | 共发现1个bug，目前状态待办的 |
...
1.  测试过程中共发现3个bug，1个确认非问题取消，2个目前状态待办的，其中一个由于本地未复现，由CCB裁决处理
2.  缺陷密度为2（缺陷个数）/0.924k（代码行数）=2.2（个/kloc）

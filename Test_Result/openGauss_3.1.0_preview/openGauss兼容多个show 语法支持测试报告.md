...
| 版本名称                       | 测试起始时间 | 测试结束时间 |
| ------------------------------ | ------------ | ------------ |
| openGauss 3.0.0 build 801d945a | 2022-09-023  | 2022-09-27   |
|                                |              |              |
...
openGauss兼容多个语法共计执行用例30条，主要覆盖功能测试、性能测试及资料测试。功能测试主要是用show grants语法对用户权限和函数/存储过程/触发器的相关信息进行显示。性能测试覆盖创建对象在10万以内及超过10万的查询时间，资料测试覆盖校验资料的描述是否正确，示例的执行结果是否成功。累计发现问题单1个，问题已解决并回归通过，无遗留风险，整体质量良好。
...
| 测试步骤                                                     | 测试结果                      |
| ------------------------------------------------------------ | ----------------------------- |
| 1.新建普通用户或管理员用户，<br/>2.新建表/视图/序列/数据库/域/数据封装器/外部服务器/函数/过程语言等测试对象<br/>3.对测试对象进行show grants查询信息验证<br/> | 执行9条用例，执行结果符合预期 |
...
| 测试步骤：                                                   | 测试结果                       |
| ------------------------------------------------------------ | ------------------------------ |
| 1.新建触发器，<br/>2.对触发器进行show grants查询相关信息验证<br/> | 执行10条用例，执行结果符合预期 |

### 4.4 测试执行统计数据

| 版本名称                       | 测试用例数 | 用例执行结果              | 发现问题单数 |
| ------------------------------ | ---------- | ------------------------- | ------------ |
| openGauss 3.0.0 build 801d945a |            | Passed：30<br />Failed：3 | 3            |
| openGauss 3.0.0 build 4cd59af8 |            | Passed：30<br />Failed：0 | 0            |

数据说明

1.  测试过程中共发现1个bug，均已解决且回归通过
2.  缺陷密度为1（缺陷个数）/1.309k（代码行数）=0.764（个/kloc）

### 4.5 后续测试建议

1. openGauss目前不支持grant column查询、对大对象进行acl查询，建议后期支持后测试。

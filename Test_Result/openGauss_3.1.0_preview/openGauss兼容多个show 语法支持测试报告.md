...
openGauss兼容多个语法共计执行用例30条，主要覆盖功能测试、性能测试及资料测试。功能测试主要是用show grants语法对用户权限和函数/存储过程/触发器的相关信息进行显示。性能测试覆盖创建对象在10万以内及超过10万的查询时间，资料测试覆盖校验资料的描述是否正确，示例的执行结果是否成功。累计发现问题单2个，1个问题已解决并回归通过，1个问题已解决待转测回归，无遗留风险，整体质量良好。
...
| ------------------------------ | ---------- | ------------------------- | ------------ |
| openGauss 3.0.0 build 801d945a |            | Passed：30<br />Failed：2 | 2            |

数据说明

1.  测试过程中共发现2个bug，均已解决且回归通过

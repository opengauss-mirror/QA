...
|    2022.12.29        |    V1.2         |   修改问题单状态       |    qiuying     |
...
CM实现自定义资源管理，共计执行77条用例，主要覆盖了功能测试、可靠性和资料测试。功能测试覆盖对自定义资源ss-proxy/zookeeper/openlookeng的状态进行监控和异常启停，json文件的配置，使用json文件对所需监控资源进行配置，实现CM对自定义资源的管理。可靠性测试覆盖在CPU、磁盘、网络异常时，CM是否可以正常管理资源。资料测试覆盖校验资料的描述及示例的执行结果是否成功。累计发现缺陷单5个，5个问题已解决且回归通过，整体质量良好。
...
| 测试步骤：                                          | 测试结果                                     |
| --------------------------------------------------- | -------------------------------------------- |
| 1. 集群进行升级、扩缩容操作<br/>2. 查看资源状态正常 | 执行10条用例，未发现问题，验收通过 |
...
| ------------------------------ | ---------- | ----------------------- | ------------ |
| openGauss 3.0.0 build b2c6e04e | 73         | Passed：73<br>Failed：25 | 5            |
| openGauss 3.0.0 build 8198a77b | 回归issue：25         | Passed：24<br>Failed：1 | 0            |
| openGauss 3.1.0 build 2c0ccaf9 | 回归issue：3         | Passed：1<br>Failed：2 | 0            |
| openGauss 3.1.0 build 2c0ccaf9 | 75         | Passed：75<br>Failed：0 | 0            |

*数据项说明：*
* 累计发现缺陷单5个，5个缺陷已解决且回归通过
* 失败用例已在后续问题修复后，回归issue执行通过
* 缺陷密度为5(缺陷个数)/1.5k(代码行数)=3.3(个/kloc)。

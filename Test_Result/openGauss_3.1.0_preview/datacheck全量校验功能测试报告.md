...
datachecker校验工具由MySQL(源端)，kafka，datachecker、openGauss(宿端)这几部分组成。datachecker工具先将源端与宿端的数据抽取，抽取完成后在使用校验服务进行两端的数据对比，然后对比成功后生成结果文件，完成数据校验。
...
| 硬件型号       | 硬件配置信息                                                 | 备注 |
| -------------- | ------------------------------------------------------------ | ---- |
| OpenStack Nova | CPU:  Intel(R) Xeon(R) Gold 6278C CPU @ 2.60GHz<br>内存：31GB<br>硬盘：523G<br>OS：CentOS Linux release 7.6.1810 (Core)<br> |      |

# 3     测试结论概述

## 3.1   测试整体结论

datachecker数据校验工具（全量）共计执行43条用例，主要覆盖了功能测试、可靠性测试、资料测试。功能测试覆盖数据类型测试；可靠性测试注入了网络延时、磁盘满、cpu满等故障进行工具功能测试。资料测试中描述恰当，示例正确无误。共计发现10个问题，其中9个问题已解决，1个问题转需求。问题均回归通过，无遗留风险，整体质量良好。（注：目前还剩1个待修复）
...
| 测试步骤：                           | 测试结果                                                     |
| ------------------------------------ | ------------------------------------------------------------ |
| 1. 验证int、char、json等16种数据类型 | 共执行25条用例<br />共发现10个bug，6个已修复且验收通过，1个转需求 |

#### 4.1.1.2 配置测试

| 测试步骤：      | 测试结果                      |
| --------------- | ----------------------------- |
| 1. 配置文件测试 | 共执行14条用例<br />未发现bug |

#### 4.1.1.3 可靠性测试

| 测试步骤：   | 测试结果                     |
| ------------ | ---------------------------- |
| 1.可靠性测试 | 共执行4条用例<br />未发现bug |
...
| :----------------------------: | :--------: | :------------------: | :----------: |
| openGauss 3.1.0 build b30defda |     43     | Passed：43 Failed：0 |      10      |

数据说明

1. 1个issue转为需求
2. 缺陷密度为9(缺陷个数)/6.51k(代码行数)=1.38(个/kloc)

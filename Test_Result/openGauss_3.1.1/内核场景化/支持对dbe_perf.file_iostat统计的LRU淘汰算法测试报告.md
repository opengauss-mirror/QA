...
| 2023-01-18 | 3.0         | 测试报告二轮评审 | chengjiaqi1 |
...
| openGauss 3.1.0 build b658f54c | 2023-01-11   | 2023-01-11   |
...
针对视图dbe_perf.file_iostat用于记录对数据文件IO活动的统计设计的LRU淘汰算法共计执行用例2条，主要覆盖功能测试，分别根据数据创建时间的先后以及数据使用频率的大小2个方面设计用例，验证LRU淘汰算法是否成功淘汰最久不用条目。测试过程发现次要问题1个，已验收通过，无遗留风险。

| 测试活动 | 活动评价                                           |
| -------- | -------------------------------------------------- |
| 功能测试 | 根据数据创建时间的先后进行淘汰算法验证，通过 |
| 功能测试 | 根据数据使用频率的大小进行淘汰算法验证，通过 |
...
#### 4.1.2 根据数据使用的频率进行淘汰算法验证

| 测试步骤                                                     | 测试结果                      |
| ------------------------------------------------------------ | ----------------------------- |
| 1.新建数据库1，库内构造数据，查询视图dbe_perf.file_iostat数据<br>2.新建数据库2，再次构造数据，使视图dbe_perf.file_iostat数据量达到2000，再次查询视图dbe_perf.file_iostat数据<br>3.分别以不同的调用次数修改数据库1与数据库2的表数据<br>3.新建数据库3，在视图dbe_perf.file_iostat数据量达到2000的前提下反复建表，对表插入，查询数据，查询此时视图dbe_perf.file_iostat数据<br>4.分析3次查询到的dbe_perf.file_iostat数据，发现原视图中由于数据库1,2数据使用频率的不同,使用频率小的那个库的表数据被新建库3的表数据替换| 执行1条用例，执行结果符合预期 |
...
| openGauss 3.1.0 build b658f54c | 2          | Passed：2<br />Failed：0 | 0            |

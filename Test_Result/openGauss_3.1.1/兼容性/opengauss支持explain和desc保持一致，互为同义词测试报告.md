...
| 测试步骤                           | 测试结果 |
| ---------------------------------- | -------- |
| 资料叙述准确，示例正确，约束全面。 | 发现1个问题，已修改并验收通过     |
...
1.  累计发现2个缺陷，1个为资料缺陷，1个为模块交互replace into模块缺陷，均已解决并回归通过。
2.  以上缺陷为资料缺陷和非本特性引入缺陷，不计入代码缺陷密度，代码缺陷密度为0（缺陷个数）/0.56kloc（代码行数）=0（个/kloc）

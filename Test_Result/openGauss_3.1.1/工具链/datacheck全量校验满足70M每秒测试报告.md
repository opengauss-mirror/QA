...
| 场景 |                  场景描述                  | 平均校验速度(MB/S) |       校验结果        |
| :--: | :----------------------------------------: | :----------------: | :-------------------: |
|  1   | 3张表，每张表数据条数1亿，数据总量132.96GB |      <22MB/S       | 只输出了表3的校验结果 |
...
| ------------------------------ | :--------- | :--------------------- | ------------ |
| openGauss 3.1.0 build 4e931f9a | 16         | Passed:14<br/>Failed:2 | 6            |

数据说明：

1. datacheck全量校验满足70M/S测试共计16条用例，执行通过14条，共计发现6个bug
2. 缺陷密度为6(缺陷个数)/2.969k(代码行数)=2.02(个/kloc)

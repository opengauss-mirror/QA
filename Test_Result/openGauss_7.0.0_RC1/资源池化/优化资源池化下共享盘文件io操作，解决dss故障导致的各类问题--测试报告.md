...
| ---- | ---------- | ------------------------------------------------------------ | -------------------- |
| 1    | 性能测试   | tpcc看护资源池化性能，不低于性能基线120w，实际测试性能120w+  | YES                  |
| 2    | 可靠性测试 | 在数据库进行io操作过程中（运行tpcc），手动执行kill掉cms、dssserver、down掉磁阵、断网、断电、主备切换、failover等操作。数据库日志对应的出现告警日志，不coredump。 | YES                  |

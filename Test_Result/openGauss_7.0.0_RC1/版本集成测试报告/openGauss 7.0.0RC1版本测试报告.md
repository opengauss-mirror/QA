...
| HTAP行列融合支持内存借用                                     | 测试在单机和传统主备场景下，基于鲲鹏超节点的远端内存借用特性，其在HTAP行列融合列缓存节点实现了列存数据的“本地内存-远端内存-本地磁盘”的三级缓存，主要包含功能测试、可靠性测试及性能测试，输出测试用例66个，执行测试共2轮，发现issue共1个，均已修复并回归通过，无遗留问题，整体开发质量良好。 | 分区表主分区不承载实际数据，主备场景主节点不存储列存数据，不需要使用远端内存借用功能。 | <font color=green>▮</font> | 无       |

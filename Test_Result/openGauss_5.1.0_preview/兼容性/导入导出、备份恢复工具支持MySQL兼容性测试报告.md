...
导入导出、备份恢复工具支持M\*兼容特性，共计执行70个用例，主要覆盖了功能测试和资料测试。功能测试覆盖了逻辑导入导出工具（gs_dump、gs_dumpall、gs_restore）和物理备份恢复工具（gs_basebackup、gs_probackup）支持M*兼容特性，导入导出数据一致；资料测试验证描述是否准确，示例的执行结果是否正确。测试发现6个问题，6个已解决且回归通过，整体质量较好。
...
| 测试步骤           | 测试结果               |
| ------------------ | ---------------------- |
| 资料的描述是否正确 | 发现2个bug，已回归通过 |

## 4.2 测试执行统计数据

| 版本名称                       | 测试用例数 | 用例执行结果         | 发现问题单数 |
| ------------------------------ | ---------- | -------------------- | ------------ |
| openGauss 5.0.0 build 90dad678 | 70         | Passed: 67 Failed: 4 | 6            |
| openGauss 5.1.0 build 338028e0 | 4          | Passed: 4 Failed: 0  | 0            |
| openGauss 5.1.0 build ad27d72a | 1          | Passed: 1 Failed: 0  | 0            |

数据项说明：

- 累计发现缺陷单6个，6个缺陷均已解决且回归通过，其中资料单2个

...
| 测试步骤                                                     | 测试结果                       |
| ------------------------------------------------------------ | ------------------------------ |
| 1. 在主机和备机上测试DDL、DML和DCL语法sql在事务或者非事务中执行，验证sql执行总时间和子时间之和的误差不超过10%；<br/>2. 测试主备switchover/failover之后，执行sql总时间与子时间之和的误差不超过10%；<br/>3. 测试PBE和sql by pass场景中sql执行总时间和子时间之和的误差不超过10%。 | 执行测试用例96条<br />测试通过 |

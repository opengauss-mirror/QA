...
（2）CDC 只同步数据，不会同步表结构，目前也不支持 DDL 的语句同步。

（3）CDC 增量阶段会按照事务的维度输出数据， 如果要开启 XA 事务的兼容，则 openGauss 和 ShardingSphere-Proxy 都需要 GLT 模块

（4）在存在超大事务的情况下，目前的处理逻辑可能会导致 CDC 所在的进程 OOM，后续可能会强制截断。

（5）满足推送的条件是满足了一定大小的数据量或者到了一定的时间间隔（目前是 300ms），在处理 XA 事务时，收到的多个分库增量事件超过了 300ms，可能会导致 XA 事务被拆开推送。
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | -------- | ------------------------------------------------------------ | -------- |
| [29259](https://github.com/apache/shardingsphere/issues/29259) | cdc server端异常，比如kill-9 ss-proxy后，cdc任务状态active仍为true，如何在异常后检测job状态需要考虑 | 次要     | 涉及sharding集群ZK数据更新，后续考虑如何优化；规避措施：可手工更新ZK状态 | 打开     |
| [29276](https://github.com/apache/shardingsphere/issues/29276) | 同一cdcjob 多次执行异常，两个客户端进程均都异常（全量数据未同步阶段报错概率高） | 次要     | 问题根因：elasticjob 启动和停止需要一点时间（依赖zk）；规避措施：不要使用多个client并发的订阅同一个CDC任务 | 打开     |
| [29508](https://github.com/apache/shardingsphere/issues/29508) | 目前cdc增量，如果xa事务量较大，读取时间超过300ms，则无法保障事务完整性。目前隐式事务csn无法增加，导致无法无限制等待。 | 次要     | 目前隐式事务CSN未进行推进，同步过程以CSN事务为单位会导致事务扩大，无超时推进截断会导致超大事务。待GLT隐式事务功能完善后，优化该场景 | 打开     |
| [29554](https://github.com/apache/shardingsphere/issues/29554) | 目前仅支持mppdb_decoding解码方式，需要配置并行解码           | 次要     | 目前整体流程无问题，并行解码适配可以提高性能。后续以需求跟踪优化 | 打开     |
...
| 17   | [29508](https://github.com/apache/shardingsphere/issues/29508) | 次要     | 目前cdc增量，如果xa事务量较大，读取时间超过300ms，则无法保障事务完整性。目前隐式事务csn无法增加，导致无法无限制等待。 | 打开     |
| 18   | [29506](https://github.com/apache/shardingsphere/issues/29506) | 主要     | cdc全量同步物理表数量较多，大于线程数量后，等待时间比较长，需要优化。 | 已完成   |
| 19   | [29554](https://github.com/apache/shardingsphere/issues/29554) | 次要     | 目前仅支持mppdb_decoding解码方式，需要配置并行解码           | 打开     |
...
​	a. 配置部署 1 个 Proxy & zookeeper 

​	b. 配置部署 16 节点 openGauss 

​	c. 部署下游消费程序

​	d. 通过 Sysbench 向 Proxy 灌入 10 个表(单表分 16 个库)，每个表 6 千 4 百万行数据，总量 6 亿 4 千万行数据，数据总量约 135 G

（2） 全量数据消费性能测试：

​	a. 启动下游消费程序，记录全量数据的消费耗时，计算 TPS 记作性能

（3） 增量数据消费性能测试：

​	a. 启动 Sysbench 发压至 Proxy，模拟增量；同时下游启动消费程序，消费数据

​	b. 记录增量数据的消费耗时，计算 TPS 记作性能指标

​	c. 记录 Sysbench 的并发度、压测时长、TPS 用于对比

#### 2.3.3.2   测试说明：

（1）目前Java堆内存都给的很大，Xmx=256g，以便于排除 JVM 相关的限制；

（2）涉及到CDC 的规则配置最优如下，内存队列的值和 batchSize 会影响到消费的速率。测试不同维度性能趋势时，只修改测试项配置，其他参数默认如下：

```
SQL ALTER STREAMING RULE (READ( WORKER_THREAD=20, BATCH_SIZE=100, SHARDING_SIZE=10000000),    WRITE( WORKER_THREAD=10, BATCH_SIZE=100),    STREAM_CHANNEL ( TYPE(NAME='MEMORY',PROPERTIES('block-queue-size'='10000'))) );
```

（3）性能测试仅验收消费阶段，实际往目标库写入依赖目标端og配置；
...
说明：保证 sysbench 的 tps 都保持在 33000 左右，并且将内存队列设置一个较大的值，然后验证 batchSize 值的影响。
...
结论：batchSize 在压力测试情况下对性能有一定的影响，过大或者过小的话会导致性能下降，配置合适的读写参数，可以适当提升消费性能，这部分需要根据实际进行调优，因为和每条数据的大小也有关系。

除此之外，性能会受到网络层面的一些限制，以及逻辑复制插槽的解析性能。

######  b. block-queue-size配置对性能的影响

保证 sysbench 的 tps 都保持在 33000 左右；因为读写都通过内存队列，所以内存队列的值时比较关键的，在性能测试中如果给的太小可能会限制读的性能，从而影响到写。这里用上面最优的参数配置即（batchSize 都等于 100），控制 block-queue-size 内存队列验证对于性能的影响。
...
结论：内存队列 block-queue-size 的值必须大于 batchSize，否则对性能会有很大的影响，当大于 5 倍以上，再往上增加效果不大。



###### c. Sysbench TPS 对性能的影响

目前 Sysbench TPS 和环境有影响，存在波动的情况
...
结论：在 TPS 32000 及以下，CDC 基本没有数据堆积，当 TPS 达到 35000 的时候，消费速率逐渐跟不上了。40000的时候，会出现消息堆积的情况。
...
| shardingsphere-on-cloud Commit ID:829f4b8195121c9386b1ff1fcc5849f507358a58 | 2023/11/12   | 2023/11/15   |
...
| 软件名称  | 软件版本                                                     | 备注                                                         |
| --------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| PITR      | **shardingsphere-on-cloud**<br/>78d680f16e99b073275d3d40ef13ef468d4d06a0<br />829f4b8195121c9386b1ff1fcc5849f507358a58<br />829f4b8195121c9386b1ff1fcc5849f507358a58 | 编译使用方式：[PITR官网说明文档](https://github.com/apache/shardingsphere-on-cloud/blob/main/pitr/README-zh.md) |
| zookeeper | 3.8.0                                                        |                                                              |
| openGauss | openGauss 3.1.0 build 5e70d87b                               |                                                              |
| redis     | 7.2.0                                                        |                                                              |
...
- 部署模式：
  ...
  -  PITR工具
    ...
    - 服务端工具gs_pitr：备份、恢复、删除备份操作发起端；对部署节点及用户无要求，可以访问proxy及pitr-agent端口即可
    - 客户端工具pitr-agent：部署在opengauss数据库用户下，包括SSL密钥；服务端发起时，客户端pitr-agent需要是启动状态
...
| shardingsphere-on-cloud Commit ID:829f4b8195121c9386b1ff1fcc5849f507358a58 | 18         | Pass：10 <br />Failed：8 | 8            |
...
​	a. 16 节点 openGauss & Pitr Agent 配置、部署

​	b. Proxy & zookeeper & Pitr Cli  & Sysbench配置、部署
...
​	a. Proxy 创建新的逻辑库，注册 openGauss 对应新的物理库

​	b. 通过 Sysbench 灌入总量 1,000w 行数据至 Proxy & 后端挂载的 openGauss，分片策略同上。
...
​	a. 通过 Pitr Cli 启动全量备份恢复，调用 16 节点 Pitr Agent 进行对应 openGauss 的全量数据恢复

​	b. 通过 Proxy & openGauss 进行数据量查询，验证全量恢复成功

（7）增量恢复:

​	a. 通过 Pitr Cli 启动全量备份恢复，调用 16 节点 Pitr Agent 进行对应 openGauss 的增量数据恢复

​	b. 通过 Proxy & openGauss 进行数据量查询，验证增量恢复成功

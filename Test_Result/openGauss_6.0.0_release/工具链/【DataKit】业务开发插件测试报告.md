...
| 执行视图相关接口性能测试脚本、分析CPU、内存占用   | 1、接口响应时长符合标准<br>                                                                                          |
...
| 问题单号 | 问题描述 | 问题级别 | 当前状态      |
| -------- | -------- | -------- |-----------|
| |表设置主键引号报错|严重| PR待检视,待回归 |
| |SQL执行多行insert时消息显示影响行数问题|提示| PR待检视     |
| |websocket超时长需设置为15分钟|一般| PR待检视     |
| |本地连接_第一次连接数据库时会报错|严重| PR待检视     |
| |新建/编辑连接按钮颜色问题|提示| 关闭        |
| |SQL执行消息显示_执行成功_日志显示内容缺失及报错|一般| PR待检视     |
| |表管理_黑夜模式下样式显示问题|提示| 分析        |
| |导出表数据_设置引号之后导出数据空白|严重| PR待检视     |
...
| |新建连接字段名需要左对齐|提示| 关闭        |
| |SQL执行历史_显示问题|提示| 关闭        |
| |创建表时设置默认值、索引、约束_报语法错误|严重| PR待检视     |
| |创建外部服务器_名称为ip地址时报错提示语法错误|严重| 测试        |
| |表名为纯数字_重建索引报语法错误|一般| 关闭        |
| |部分状态为失败的SQL没有错误信息记录|一般| 合入社区      |
| |标签栏响应问题|一般| 关闭        |
| |无连接时终端选择数据库的下拉框未隐藏|提示| 关闭        |
| |[性能]表管理功能相关接口性能问题|一般| 分析        |
...
|-----|------|-------------------|------|------|-----------------------------------------|--------------|
| 1   |      | [性能]表管理功能相关接口性能问题 | 性能问题 | 一般   | 不影响业务使用。当并发量达到100时，部分接口响应较慢，平均响应时间接近3秒。 | 并发高的时候需等待响应。 |
| 2   |      | 表管理_黑夜模式下样式显示问题 | 易用性问题             | 提示                |分析| 颜色显示不便于查看，不影响业务使用，无需规避                  |
##  7.2 附件2：特性相关PR

...
|-----|------------------------------|---------------|-----------------------------------------------|---|---|--------------|
| 1   |【国创】实例监控插件性能与稳定性优化| 均已实现          | 已覆盖                                           |100%|无|无|
| 2   |【国创】智能诊断插件SQL诊断功能支持Schema| 均已实现          | 已覆盖                                           |100%|无|无|
| 3   |【国创】智能诊断插件SQL诊断功能支持非ROOT用户| 均已实现          | 已覆盖                                           |100%|无|无|
| 4   |【国创】支持慢SQL统计功能| 均已实现          | 已覆盖                                           |100%|无|无|
| 5   |【国创】告警监控插件提供插件告警接口| 均已实现          | 已覆盖界面功能。对于内部接口，暂无相应测试机制，需使用者在自行开发时参考特性设计进行调用。 |100%|无|无|
| 6   |【国创】支持告警屏蔽| 均已实现          | 已覆盖                                           |100%|无|无|
...
| 特性                         |问题数|代码量| 缺陷率  |
|----------------------------|----|---|------|
|【国创】实例监控插件性能与稳定性优化|6|1800| 3.33 |
|【国创】支持慢SQL统计功能、【国创】智能诊断插件SQL诊断功能支持Schema|4|2836| 1.41 |
|【国创】智能诊断插件SQL诊断功能支持非ROOT用户|2|398| 5.03 |
|【国创】告警监控插件提供插件告警接口|6|1560| 3.85 |
|【国创】支持告警屏蔽|5|1641| 3.05 |
...
|-----|-----------------------------------------------------------------------------------------------------------|------|-----------|
| 1   |【国创】实例监控插件性能与稳定性优化       | 测试实例监控接口性能、资源占用。测试长期监控时是否有掉点。测试安装多个prometheus时，prometheus启动/停止后，采集时间段内的数据情况。测试代理或prometheus异常情况下及恢复后的数据采集 | 是    |
| 2   |【国创】智能诊断插件SQL诊断功能支持Schema | 测试Schema为public时的SQL诊断任务执行，测试 Schema为其他模式时的SQL诊断任务执行                                                      | 是    |
| 3   |【国创】智能诊断插件SQL诊断功能支持非ROOT用户 | 测试使用ROOT用户安装/启动/停止/卸载，并执行SQL诊断任务。测试 使用非ROOT用户安装/启动/停止/卸载，并执行SQL诊断任务                                       | 是    |
| 4   |【国创】支持慢SQL统计功能           | 测试慢SQL统计列表、趋势图的查询/排序/统计数据结果正确性，测试慢SQL明细列表查询/排序/统计数据结果正确性                                                  |    是  |
| 5   |【国创】告警监控插件提供插件告警接口       | 测试插件告警规则的新建/编辑/查看/删除，测试插件告警模板的新建/编辑/查看/删除，测试插件告警配置，测试插件告警记录的查询/导出，测试插件告警通知                                | 是    |
| 6   |【国创】支持告警屏蔽               | 测试告警屏蔽规则的 新建/编辑/查看/删除，测试告警屏蔽生效/失效。测试告警屏蔽生效时、若告警状态恢复，告警记录可正常结束。                                            | 是                                                                                                         |
...
| -------- |-----------|-------|-------|-------|------------------------------------|
| openGauss 6.0.0  | 19        | 184   | 184   | 121   | 总问题数119/7kloc=17,遗留问题数6/7kloc=0.86 |
|          |           |       |       |       |                                    |

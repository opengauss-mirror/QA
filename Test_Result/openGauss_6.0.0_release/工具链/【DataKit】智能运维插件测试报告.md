...
本项性能测试 设置并发用户数为100，每1秒启动20个用户，执行表管理相关接口的测试脚本120秒，停止后等待时间为180秒。
...
| 执行实例监控相关接口性能测试脚本、分析CPU、内存占用。 | 1、大部分接口响应时长符合标准，其中asp的接口、实例监控首页TOP进程接口的平均响应时长超过预期值，达3s以上。需要优化。<br> 2、脚本执行期间（第2s到第5分钟2秒），平台、平台数据库、被测数据库的CPU和内存占用均增长，最大值未超过预期值。脚本执行结束后会降低。<br>被测数据库的CPU在390%~644.2%（除以核数后为48.75%~80.525%）之间波动，内存在1945.6M~3174.4M之间波动，CPU占用和内存占用均小于70%，符合预期。<br>一级prometheus的最大CPU占用为250.5%(除以核数后为31.31%)，内存最大占用为143.85M；<br>二级prometheus的最大CPU占用为50%(除以核数后为6.25%)，内存最大占用为867.13M；<br>二级prometheus的最大CPU占用为27.5%(除以核数后为3.44%)，内存最大占用为58.24M；<br>实例监控代理的最大CPU占用为93.1%(除以核数后为11.64%)，内存最大占用为504.95M。<br> |
...
| -------- |-----------|-------|-------|-------|-------------------------------------|
| openGauss 6.0.0  | 19        | 184   | 184   | 25    | 总问题25/62kloc=0.4,遗留问题数2/62kloc=0.03 |
|          |           |       |       |       |                                     |
...
|-----|------|-------------------|------|-----|-----------------------------------------|------------------------------------|
|1|BUG20240910652227|告警记录页面样式与原型设计不符|旧功能问题|提示|搜索条件之间的间隔宽度过大、分隔符未居中。不影响功能。| 暂无需规避。                             |
|2|BUG20240906648013|告警屏蔽接口性能优化|性能问题|一般|并发量大时接口性能较差，不影响功能| 暂无需规避                              |
|3|BUG20240905647587|停止后立马启动|旧功能问题|一般|难以复现，不影响功能| 停止以后等1秒后再点击启动                      |
|4|BUG20240819644439|实例监控_指标没有数据|旧功能问题|严重|ssh连接不上，可能是系统资源或者其他原因| 重启代理端                              |
|5|BUG20240812644305|日志检索和sql诊断的安装部署_中文显示编码问题|旧功能问题|提示|输入中文会有问题，不影响功能| 不输入中文                              |
|6|BUG20240806643051|升级_二级prometheus一直在启动中|兼容性问题|严重|难以复现，主要是因为二级prom版本比较旧| 重新安装二级prom或者手动将配置写入prom，然后重新启动prom |
|7|BUG20240806641844|升级_启动exporter报错|兼容性问题|严重|难以复现，主要是因为二级prom版本比较旧| 重新安装二级prom或者手动将配置写入prom，然后重新启动prom |
|8|BUG20240729139148|在liunx服务器手动启动exporter，无法自动分配到prom|旧功能问题|提示|服务器手动启动exporter，无法更新exporter的状态（修改数据库），不影响功能| 暂无需规避                              |
|9|BUG20240708136237|elastic在线安装_未下载文件|旧功能问题|一般|难以复现，不影响功能| 暂无需规避                              |
|10|BUG20240705136712|单集群状态为未知时_主备延迟列表中节点名称未知|旧功能问题|一般||
|11|BUG20240628134839|集群监控_主备切换后集群详情中的图表报错|旧功能问题|一般|切换主备后，promethues中查询不到相关数据| 切换后等待30秒刷新                         |
|12|BUG20240624134735|安装部署列表接口格式问题|接口规范问题|提示|旧功能未按规范格式，不影响功能| 暂无需规避                              |
|13|BUG20240621134388|【掉点相关】实例数据采集刷新问题|旧功能问题|严重|可能由于网络不好，导致某些时间没有采集到数据，等网络稳定后就不会该问题| 暂无需规避                              |
|14|BUG20240620135500|【掉点问题】33实例监控cpu使用率显示数据问题|旧功能问题|严重|可能由于网络不好，导致某些时间没有采集到数据，等网络稳定后就不会该问题| 暂无需规避                              |
|15|BUG20240620135221|elastic出现异常、占用高|性能问题|一般|日志数据量较大时CPU占用较高，不影响功能| 暂无需规避                              |

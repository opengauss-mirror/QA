...
openGauss 6.0.0 版本整体测试按照release-manager团队的计划，版本测试规划采取16+3+1的测试方式，即16轮系统测试+3轮集成验证+1轮回归测试的策略，实际完成了16轮系统测试+3轮集成验证+1轮回归测试：
...
|【国创】实例监控插件性能与稳定性优化| 测试采集数据稳定性。 执行5个用例，发现1个问题，问题已修复。                           | 无                                         |<font color=yellow>▲</font>||
...
针对openGauss 6.0.0 版本，规划的升级路径如下表所示。
...
| Domain       | 测试活动                                                                                                                                              | 测试结论    |
| ------------ |---------------------------------------------------------------------------------------------------------------------------------------------------| ----------- |
| 操作系统兼容 | 支持在arm+openEuler 20.03 LTS上部署 DataKit平台及业务开发插件和智能运维插件                                                                                             |
| 数据库兼容  | 智能运维插件支持监控openGauss 3.0.0、openGauss 5.0.1、openGauss 6.0.0。 业务开发插件支持连接openGauss 3.0.0、openGauss 5.0.1、openGauss 6.0.0，其中覆盖率功能仅在openGauss 6.0.0版本可用 |
...
openGauss 6.0.0
版本共发现问题9个，有效问题9个，无效问题0。修复问题回归测试结果正常，版本整体质量良好。遗留6个问题详细分布见下表:
...
| openGauss 6.0.0  Test round 5 | 2024-9-12 | 2024-9-18 | 3     | 0     |
...
| 序号  | issue号 | 问题简述                                       |优先级| 状态  |
|-----|--------|--------------------------------------------|---|-----|
| 1   |IARQ21|缺陷|【工具功能】【测试版本：6.0.0】DataKit-智能运维_实例监控_指标数据问题| 次要  |修复中|
| 2   |IAR6A2|缺陷|【集群监控】_主备切换后集群详情中的图表报错| 次要  |待回归|
| 3   |IAR657|缺陷|【业务开发】编辑表列_勾选是否唯一并保存_提示表已经存在| 次要  |待回归|
| 4   |IAONY7|缺陷|告警监控插件测试缺陷| 主要  |待回归|
| 5   |IAI06J|缺陷|【国创】业务开发插件编辑表数据前端优化| 次要  |已验收|
| 6   |IAIO5M|缺陷|【国创】业务开发插件编辑表数据黑夜模式下样式显示问题| 次要  |已验收|
| 7   |IAHX49|缺陷|【国创】业务开发插件无法导出表数据| 主要  |已完成|
| 8   |IABORE|缺陷|日志检索插件缺陷| 次要  |已答复|
| 9   |IAB8MN|缺陷|智能诊断插件缺陷| 次要  |已验收|

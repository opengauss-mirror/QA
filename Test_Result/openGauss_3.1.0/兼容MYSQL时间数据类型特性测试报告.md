...
基于插件dolphin兼容MYSQL时间数据类型，共计执行183条用例，主要覆盖了功能测试和资料测试。功能测试覆盖数据类型的取值范围、缺省精度、允许输入的格式、时区处理方式、类型交互使用等。资料测试覆盖校验资料的描述是否正确，示例的执行结果是否成功。累计发现缺陷单20个，经评审已取消2个issue，其余缺陷均已解决，回归通过，无遗留风险，整体质量良好。

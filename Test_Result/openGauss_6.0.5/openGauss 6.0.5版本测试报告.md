...
**Keywords 关键词**：openGauss 6.0.5

**Abstract 摘要**：主要是描述了openGauss 6.0.5版本的整体测试情况，给出本阶段的测试范围、结果、分析及质量评价，同时对测试活动进行回顾总结。
...
openGauss 6.0.5版本回合了修复的若干关键缺陷代码。本文主要描述了openGauss 6.0.5版本整体测试情况，重点从特性质量、专项测试和问题单统计等维度展开叙述。综合来看，oopenGauss 6.0.5版本整体质量良好。

# 测试版本说明

| 版本名称             | 测试起始时间 | 测试结束时间 |
| -------------------- | ------------ | ------------ |
| openGauss 6.0.5 B001 | 2026-04-14    | 2026-04-30   |
| openGauss 6.0.5 B002 | 2026-05-07    | 2025-05-13   |
...
openGauss 6.0.5版本整体测试按照release-manager团队的计划，版本测试规划采取2轮测试方式，完成了1轮系统测试+1轮回归测试：

第1轮完成所有回合的特性测试，同时对继承特性开展第1轮的全量测试，覆盖功能、可靠性、安全、性能、长稳、升级等测试，同步全量CI连跑。第2轮验收回合版本的问题单，并对特性测试、专项测试给出结论。

openGauss 6.0.5版本按照测试策略完成了全量功能验证和专项测试（性能、可靠性、兼容性、安全和资料等），所有测试任务均按计划完成。本版本验收需求1个，问题61个，完成率100%。

# 版本详细测试结论

openGauss 6.0.5版本详细测试内容包括：
...
| ---------------------------------------------- | ---------------------------- | --------------- | -------------------------- | -------- |
| openGauss整机资源瘦身 -- CPU、内存、IO资源优化 | 转测无问题，内存优化符合预期 | 基于4K page编译 | <font color=green>▮</font> | 无       |


### 问题单验收结论

对版本所有回合的问题验收，测试发现7个均已验收：

| 序号 | 问题描述                                                     | ISSUE链接                    | 验收状态         |
| ---- | ------------------------------------------------------------ |  ---------------------------------------------------| -------------- |
1	|	[Bug]: 【升级】5.0.5升级到6.0.5版本，升级报错	|	https://gitcode.com/opengauss/openGauss-server/issues/8116	|	已验收	|
2	|	[Bug]: 【升级】【传统】6.0.5升级7.0.0LTS，升级失败，数据库core	|	https://gitcode.com/opengauss/openGauss-server/issues/8118	|	已验收	|
3	|	[Bug]: A库下建依赖function的视图，重建相同的function后视图，引用表 tmp_reuslt	|	https://gitcode.com/opengauss/openGauss-server/issues/8120	|	已验收	|
4	|	[Bug]: 【6.0.5】gs_guc -Z cmserver -N all -I all -c upgrade_from="0"失败	|	https://gitcode.com/opengauss/CM/issues/262	|	已验收	|
5	|	[Bug]: b_format_behavior_compat_options = ''时，不支持tinyblob，mediumblob等前缀索引	|	https://gitcode.com/opengauss/openGauss-server/issues/8129	|	已验收	|
6	|	[Bug]: 6.0.5补丁创建订阅端失败	|	https://gitcode.com/opengauss/openGauss-server/issues/8131	|	已验收	|
7	|	[Bug]: realtime_build_log_ctrl_status函数从500升级到605和直装605存在差异	|	https://gitcode.com/opengauss/openGauss-server/issues/8137	|	已验收	|
...
openGauss 6.0.5 版本可靠性测试覆盖：硬件故障/操作系统故障/数据库系统故障/人为因素故障/RTO/工具等6个故障注入类测试及1个长时间负载测试。
...
| ------------ | -------------------------------------------- | ---------- | ----------------- | ------------------------------------------- |
| TPCC         | 2P（Taishan 200 2280 7260）单节点 分区表     | 150万      | 此即为release基线 | 单节点1H在极限场景配置下tpmC为**152.8**W+   |
|              | 2P（Taishan 200 2280 7260）单节点 非分区表   | 145万      | 此即为release基线 | 单节点8H在极限场景配置下tpmC为**151.7**W+   |
|              | 2P（Taishan 200 2280 7260）一主一备 分区表   | 120万      | 此即为release基线 | 一主一备1H在极限场景配置下tpmC为**134.5**W  |
|              | 2P（Taishan 200 2280 7260）一主一备 非分区表 | 120万      | 此即为release基线 | 一主一备8H在极限场景配置下tpmC为**133.7**W+ |
|              | 4P（Taishan 200 2480 7260）单节点 1H         | 230万      | 此即为release基线 | 单节点在极限场景配置下tpmC为**232**W+       |
...
| **指标大项** | **指标小项**                                 | **指标值** | **说明**          | 测试结论                                    |
| ------------ | -------------------------------------------- | ---------- | ----------------- | ------------------------------------------- |
| TPCC         | 2P（Taishan 200 2280 7260）主机读写备机只读  | 220万      | 此即为release基线 |  266.9W                     |
|              | 2P（Taishan 200 2280 7260）主机读写备机空载  | 120万      | 此即为release基线 |  126W                       |
| sysbench     | 2P（Taishan 200 2280 7260）一主一备 分区表   | 37887      | 此即为release基线 |  39507 (同机器6.0.0值37887) |
|              | 2P（Taishan 200 2280 7260）一主一备 非分区表 | 35000      | 此即为release基线 |  45133                      |


### 安全测试

openGauss 6.0.5 版本安全测试覆盖：
...
openGauss 6.0.5版本所有适用的安全扫描和安全测试均已执行，整体质量良好，风险可控。

### 兼容性测试

#### 升级测试

针对openGauss 6.0.5版本，规划的升级路径如下表所示。共进行2轮测试，测试整体情况：

1、openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 升级到openGauss 6.0.5成功，升级失败或者升级未提交，可以成功回滚；

2、openGauss 6.0.5升级到openGauss master成功，回滚成功；

3、升级成功后，除行存压缩特性，其余特性功能运行正常。

| 升级路径                                                     | 测试结论 |
| ------------------------------------------------------------ | -------- |
| openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 不带cm灰度升级到openGauss 6.0.5不带cm | 测试通过 |
| openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 不带cm灰度升级到openGauss 6.0.5带cm | 测试通过 |
| openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 带cm灰度升级到openGauss 6.0.5带cm | 测试通过 |
| openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 不带cm灰度升级到openGauss 6.0.5不带cm，回滚后，再升级提交 | 测试通过 |
| openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 不带cm灰度升级到openGauss 6.0.5带cm，回滚后，再升级提交 | 测试通过 |
| openGauss 5.0.1/openGauss 5.0.3/openGauss 6.0.0 - 6.0.3 带cm灰度升级到openGauss 6.0.5带cm，回滚后，再升级提交 | 测试通过 |
| openGauss 6.0.5不带cm升级到openGauss master不带cm            | 测试通过 |
| openGauss 6.0.5不带cm升级到openGauss master带cm              | 测试通过 |
| openGauss 6.0.5带cm升级到openGauss master带cm                | 测试通过 |
| openGauss 6.0.5不带cm升级到openGauss master不带cm，回滚后，再升级提交 | 测试通过 |
| openGauss 6.0.5不带cm升级到openGauss master带cm，回滚后，再升级提交 | 测试通过 |
| openGauss 6.0.5带cm升级到openGauss master带cm，回滚后，再升级提交 | 测试通过 |

#### 硬件兼容

| Domain       | 测试活动                                                     | 测试结论 |
| ------------ | ------------------------------------------------------------ | -------- |
| 服务器兼容   | 在X86（Intel(R) Xeon(R) Gold）/鲲鹏（Kunpeng 920）服务器上安装部署openGauss 6.0.5数据库 | 测试通过 |
| 存储设备兼容 | 在本地盘（SAS、SATA和SSD）及云盘上安装部署openGauss 6.0.5数据库 | 测试通过 |

#### 软件兼容

| Domain       | 测试活动                                                     | 测试结论 |
| ------------ | ------------------------------------------------------------ | -------- |
| 操作系统兼容 | 在X86+Centos7.6/X86+openEuler 20.03 LTS/X86+openEuler 22.03 LTS/环境下，安装部署openGauss 6.0.5数据库 | 测试通过 |

# 问题单统计

openGauss 6.0.5版本共发现问题7个，无效问题0个，该版本规划解决的问题均已合入，验收通过：

| 版本名称             | 测试起始时间 | 测试结束时间 | 有效问题数 | 无效问题数 |
| -------------------- | ------------ | ------------ | ---------- | ---------- |
| openGauss 6.0.5 B001 | 2026-04-14     | 2026-04-30    | 7         | 0          |
| openGauss 6.0.5 B002 | 2026-05-07     | 2026-05-13    | 0          | 0          |

本次测试共2轮，2026.04.14正式进入openGauss 6.0.5测试，B001合入问题单验收、全量测试，暴露出很多升级问题的元数据问题， B002对功能问题全部解决。元数据不影响功能问题遗留到6.0.6闭环。

...
openGauss支持openEuler 22.03LTS系统共执行35个用例，主要覆盖功能测试、性能测试和长稳测试，主要包含在x86架构和arm架构的openEuler  22.03LTS系统编译数据库内核、OM、CM、JDBC/ODBC/python驱动、libpq以及迁移工具，以不同方式的安装数据库并执行基本SQL用例，以及TPCC性能测试、长稳测试和资料测试。共发现0个问题，基本功能可用，整体质量良好。

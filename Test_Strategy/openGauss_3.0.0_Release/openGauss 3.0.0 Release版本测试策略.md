...
| 2      | 【openGauss 3.0.0 LTS】openGauss分布式解决方案               | 验证shardingSphere针对openGauss做协议和驱动适配，以及性能要求 | 验证改进后的jdbc支持ShardingSphere对接openGauss，并且功能正常，且性能测试满足CPU使用率下降10% |        |
...
| 4      | 【openGauss 3.0.0 LTS】openGauss支持发布订阅的多地多活       | 验证发布端、订阅端的数据同步后的一致性，关注故障场景和大并发场景 | 关注发布端故障时的恢复手段，以及避免订阅端的重复订阅         |        |
...
| 10     | 【openGauss 3.0.0 LTS】openGauss支持Global system cache      | 验证新增GUC参数、新增视图和新增函数是否生效                  | 验证global内存淘汰、内存清理函数功能等                       |        |
...
openGauss 3.0.0 Release版本按照社区release-management团队既定的版本计划，共有8轮测试，按照社区研发模式，所有的需求在2022年2月20日前完成合入。版本计划规划9轮测试，采取5+3的测试方式，即5轮系统测试+3轮集成验证的策略。
...
1. 重点关注新特性的规格验收
2. 继承功能通过自动化看护
4. 专项测试：兼容性测试（不含升级兼容性和数据库兼容性测试）,性能测试（5220性能指标验收），资料测试，安全测试

...
| 44   | 【openGauss 5.0.0 Release】外键支持关联到普通索引            |  主要验证外键支持关联到普通索引与M执行结果是否一致，且保证一些特殊场景的应用           | 1.验证支持对唯一索引创建外键<br/>2.验证创建非唯一性索引的外键合理报错键<br/>3.验证对唯一索引创建外键，UPDATE之后对表中内容的影响键<br/>4.验证对唯一索引创建外键，DELETE 之后对表中内容的影响<br/>5.验证BFORMAT模式对唯一索引创建外键的影响<br/> 6.验证BFORMAT模式对非唯一索引创建外键的影响 |     |
